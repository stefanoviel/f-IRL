...
for q in "${q_pairs[@]}"; do
    for seed in "${seeds[@]}"; do
        for q_std_clip in "${q_std_clip[@]}"; do
            ...
            (python -m firl.irl_samples --config configs/samples/agents/ant.yml --num_q_pairs $q --seed $seed --q_std_clip $q_std_clip> outputs/run_q${q}_seed${seed}_clip${q_std_clip}.log 2>&1) &
